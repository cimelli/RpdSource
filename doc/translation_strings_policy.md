...
This document provides guidelines for internationalization of the RPDCHAIN Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to RPDCHAIN network</source>
        <translation>
            <numerusform>%n active connection to RPDCHAIN network</numerusform>
            <numerusform>%n active connections to RPDCHAIN network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `rpdchain_en.ts` remains unchanged.
