...
The build script we'll be using is contained in the RPDCHAIN github repository ([contrib/gitian-build.py](https://github.com/rpdchain-project/rpdchain/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the RPDCHAIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/RPDCHAIN-Project/RPDCHAIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, rpdchain-detached-sigs, and rpdchain GitHub repos.
