...
*Setup instructions for a gitian build of RPDCHAIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the RPDCHAIN
...
to the RPDCHAIN GitHub release page.
...
- [Building RPDCHAIN Core](#building-rpdchain-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/rpdchain-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/rpdchain-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/rpdchain-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/rpdchain-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/rpdchain-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/rpdchain-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/rpdchain-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@rpdchain.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/rpdchain-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/rpdchain-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/rpdchain-osx-*-build.assert
```
