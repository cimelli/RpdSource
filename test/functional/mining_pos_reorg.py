...
class ReorgStakeTest(RpdChainTestFramework):
    ...
    def check_money_supply(self, expected_rpd, expected_zrpd):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected RPD and zRPD supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_rpd))
            for denom in node['zRPDsupply']:
                assert_equal(node['zRPDsupply'][denom], DecimalAmt(expected_zrpd[denom]))
    ...
    def run_test(self):
        ...
        # Check RPD and zRPD supply at the beginning
        # ------------------------------------------
        # zRPD supply: 2 coins for each denomination
        expected_zrpd_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # RPD supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zrpd_supply)
        ...
        # Verify that RPD and zRPD supplies were properly updated after the spends and reorgs
        self.log.info("Check RPD and zRPD supply...")
        ...
        expected_zrpd_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zrpd_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zrpd_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zrpd_supply)
