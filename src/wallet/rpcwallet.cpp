...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "  \"account\" : \"account\"                 (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start rpdchaind with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in v5.0. To use this command, start rpdchaind with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in v5.0. To use this command, start rpdchaind with -deprecatedrpc=accounts.");
    }
...
UniValue getaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in v5.0. To use this command, start rpdchaind with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in v5.0. To use this command, start rpdchaind with -deprecatedrpc=accounts.");
    }
...
UniValue listmintedzerocoins(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zrpdTracker->ListMints(true, fMatureOnly, true);
        ...
        for (auto m : setMints) {
            ...
            if (m.hashStake.IsNull()) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zrpdTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zrpdTracker->ListMints(true, true, true);
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Rpd to convert to zRPD\n"
...
UniValue spendzerocoin(const JSONRPCRequest& request)
{
    ...
    return DoZrpdSpend(nAmount, vMintsSelected, address_str);
}
...
UniValue resetmintzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzRPDTracker* zrpdTracker = pwalletMain->zrpdTracker.get();
    std::set<CMintMeta> setMints = zrpdTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzRPDTracker* zrpdTracker = pwalletMain->zrpdTracker.get();
    std::set<CMintMeta> setMints = zrpdTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zrpdTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const JSONRPCRequest& request)
{
    ...
    CzRPDTracker* zrpdTracker = pwalletMain->zrpdTracker.get();
    std::set<CMintMeta> setMints = zrpdTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const JSONRPCRequest& request)
{
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zrpdTracker->Add(mint, true);
        count++;
...
UniValue setzrpdseed(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setzrpdseed \"seed\"\n"
            "\nSet the wallet's deterministic zrpd seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zrpd seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzrpdseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzrpdseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzRPDWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzrpdseed(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getzrpdseed\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("getzrpdseed", "") + HelpExampleRpc("getzrpdseed", ""));
...
UniValue dzrpdstate(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "dzrpdstate\n"
    ...
    obj.push_back(Pair("dzrpd_count", nCount));
...
void static SearchThread(CzRPDWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZRPD(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzrpd(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "searchdzrpd\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("searchdzrpd", "1, 100, 2") + HelpExampleRpc("searchdzrpd", "1, 100, 2"));
    ...
    boost::thread_group* dzrpdThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzrpdThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzrpdThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zrpdTracker->GetSerialHashes());
...
UniValue spendrawzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) RPDCHAIN address to spend to. If not specified, "
    ...
    return DoZrpdSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
