// Copyright (c) 2017-2020 The RPDCHAIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zrpdwallet.h"
#include "main.h"
#include "txdb.h"
...
CzRPDWallet::CzRPDWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zrpd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRPDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRPDSeed_deprecated()) {
                    LogPrintf("%s: Updated zRPD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrpd seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrpd wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRPDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRPDCount(nCountLastUsed);
    else if (!walletdb.ReadZRPDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRPDWallet::Lock()
{
    seedMaster.SetNull();
}

void CzRPDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRPDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRPD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRPDWallet::LoadMintPoolFromDB()
{
...
void CzRPDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRPDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRPDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zrpdTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRPDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRPD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrpdTracker which also adds to database
    wallet->zrpdTracker->Add(dMint, true);
...
void CzRPDWallet::SeedToZRPD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzRPDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRPDWallet::GenerateDeterministicZRPD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRPDWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRPDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRPDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
