...
bool CLegacyZRpdStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZRpdStake from non zc-spend", __func__);
    ...
    *this = CLegacyZRpdStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zrpd stake origin", __func__);

    // All good
    return true;
}

CLegacyZRpdStake::CLegacyZRpdStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZRpdStake::GetIndexFrom()
{
...
CAmount CLegacyZRpdStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZRpdStake::GetUniqueness() const
{
...
bool CLegacyZRpdStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zRPD stake block: height %d outside range", __func__, nHeight);
