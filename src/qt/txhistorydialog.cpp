    ...
    walletModel(0)
{
    ...
    ui->txHistoryTable->setColumnHidden(1, true); // hidden sort key
    ...
    ui->txHistoryTable->setStyleSheet("QTableWidget {color:#d1d5db;background-color:transparent;} QHeaderView::section { font-weight: bold; }");
}
...
void TXHistoryDialog::UpdateConfirmations()
{
    ...
    for (int row = 0; row < rowCount; row++) {
        ...
        QIcon ic = QIcon(":/ic-pending");

        if (confirmations > 1) ic = QIcon(":/ic-check");

        if (!valid) ic = QIcon(":/ic-close-white");
        QTableWidgetItem *iconCell = new QTableWidgetItem;
        // ic = platformStyle->SingleColorIcon(ic);

        if (row % 2 == 0) {
            iconCell->setData(Qt::BackgroundRole, QColor(80,80,79));
        } else {
            iconCell->setData(Qt::BackgroundRole, QColor(60,60,59));
        }
...
void TXHistoryDialog::UpdateHistory()
{
    ...
    for (int i = 0; i < nRows; ++i)
    {
        ...
        {
            if (i % 2 == 0) {
                ui->txHistoryTable->item(i, j)->setData(Qt::BackgroundRole, QColor(80,80,79));
            } else {
                ui->txHistoryTable->item(i, j)->setData(Qt::BackgroundRole, QColor(60,60,59));
            }
        }
    }
}
