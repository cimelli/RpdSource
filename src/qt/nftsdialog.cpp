...
#include "nftsdialog.h"
#include "ui_nftsdialog.h"
...
NftsDialog::NftsDialog(QWidget *parent) :
    QDialog(parent), ui(new Ui::nftsDialog), clientModel(0), walletModel(0)
{
    // setup
    ui->setupUi(this);
...
void NftsDialog::setClientModel(ClientModel *model)
{
...
void NftsDialog::setWalletModel(WalletModel *model)
{
    this->walletModel = model;
    if (model != NULL) { } // do nothing, signals from walletModel no longer needed
}

void NftsDialog::UpdatePropSelector()
{
    LOCK(cs_tally);
    ...
    for (std::set<uint32_t>::iterator it = global_wallet_property_list.begin(); it != global_wallet_property_list.end(); ++it) {
        ...
        // Check if either available or reserved balance is equal to 1 or if 0
...
void NftsDialog::AddRow(const std::string& label, const std::string& address, const std::string& spTicker, const std::string& reserved, const std::string& available)
{
...
void NftsDialog::PopulateBalances(unsigned int propertyId)
{
...
void NftsDialog::propSelectorChanged()
{
    QString spId = ui->propSelectorWidget->itemData(ui->propSelectorWidget->currentIndex()).toString();
...
void NftsDialog::contextualMenu(const QPoint &point)
{
...
void NftsDialog::resizeEvent(QResizeEvent* event)
{
