...
#include "rpdchain-config.h" /* for USE_QRCODE */
#endif
...
void LookupAddressDialog::searchAddress()
{
    ...
    if (IsValidDestination(address)) //do what?
    {
        ...
        ui->balanceLabel->setText(QString::fromStdString(FormatDivisibleMP(GetAvailableTokenBalance(searchText, 1)) + " TOKEN"));
        // QR
        #ifdef USE_QRCODE
        ui->QRCode->setText("");
        ...
        for (pItem = 1; pItem < 12; pItem++)
        {
            ...
            for (propertyId = lastFoundPropertyIdMainEco+1; propertyId<10000; propertyId++)
            {
                ...
                if (GetAvailableTokenBalance(searchText, propertyId) > 0)
                {
                    ...
                    pName[pItem] += strprintf("", propertyId);
            ...
            if (!foundProperty)
            {
                for (propertyId = lastFoundPropertyIdTestEco+1; propertyId<10000; propertyId++)
                {
                    if (GetAvailableTokenBalance(searchText, propertyId+2147483647) > 0)
                    {
                        ...
                        if(pName[pItem].size()>32) pName[pItem]=pName[pItem].substr(0,32)+"...";
                        pName[pItem] += strprintf("", propertyId+2147483647);
        ...
        for (pItem = 1; pItem < 11; pItem++)
        {
            if (pFound[pItem])
            {
                ...
                if (pName[pItem]=="Test Token (#2)") { tokenLabel = " TOMN"; }
                if (pDivisible[pItem])
                {
