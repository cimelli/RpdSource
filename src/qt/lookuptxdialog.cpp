    ...
    if (0<=populateResult) {
        ...
        switch(populateResult) {
            case MP_TX_NOT_FOUND:
                strText += "not a valid RPDCHAIN transaction.  Please check the transaction hash "
            ...
            case MP_TX_IS_NOT_TOKEN_PROTOCOL:
                strText += "a RPDCHAIN transaction only.\n\nTip: You can use the debug console "
                           "'gettransaction' command to lookup specific RPDCHAIN transactions.";
            break;
