...
#include "qt/rpdchain/splash.h"
#include "qt/rpdchain/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("rpdchain-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in rpdchain.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in rpdchain.qrc)
...
/** Class encapsulating RPDCHAIN Core startup and shutdown.
...
/** Main RPDCHAIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (RPDCHAINGUI)
...
{
    window = new RPDCHAINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &RPDCHAINGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &RPDCHAINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(RPDCHAINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(RPDCHAINGUI::DEFAULT_WALLET);
        ...
        // RPDCHAIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &RPDCHAINGUI::handlePaymentRequest);
        connect(window, &RPDCHAINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. RPDCHAIN Core Wallet can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(rpdchain_locale);
    Q_INIT_RESOURCE(rpdchain);
    ...
    /// 6. Determine availability of data directory and parse rpdchain.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("RPDCHAIN Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("RPDCHAIN Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // rpdchain: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("RPDCHAIN Core Wallet didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
