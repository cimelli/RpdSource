...
#include "qt/rpdchain/settings/forms/ui_settingsbittoolwidget.h"
...
SettingsBitToolWidget::SettingsBitToolWidget(RPDCHAINGUI* _window, QWidget *parent) :
...
void SettingsBitToolWidget::onEncryptKeyButtonENCClicked()
{
    ...
    if (!isValidPassphrase(qstrPassphrase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: #ff5533; }");
...
void SettingsBitToolWidget::onDecryptClicked()
{
    ...
    if (!BIP38_Decrypt(strPassphrase, strKey, privKey, fCompressed)) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: #ff5533; }");
