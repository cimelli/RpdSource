...
#include "qt/rpdchain/dashboardwidget.h"
#include "qt/rpdchain/forms/ui_dashboardwidget.h"
#include "qt/rpdchain/sendconfirmdialog.h"
#include "qt/rpdchain/txrow.h"
...
DashboardWidget::DashboardWidget(RPDCHAINGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &RPDCHAINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &RPDCHAINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &RPDCHAINGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isRpd = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZRPD;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            rpd = (pair.first != 0) ? pair.first / 100000000 : 0;
            zrpd = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalRpd += pair.first;
            chartData->totalZrpd += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesRpd.append(rpd);
        chartData->valueszRpd.append(zrpd);

        int max = std::max(rpd, zrpd);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(60,60,59));
    set1->setColor(QColor(60,60,59));
