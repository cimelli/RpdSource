...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeRpdChainAmount") || reset)
        SoftSetArg("-anonymizerpdchainamount", settings.value("nAnonymizeRpdChainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rpdchain.cpp during shotdown to also remove the window positions
