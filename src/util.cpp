...
const char * const RPDCHAIN_CONF_FILENAME = "rpdchain.conf";
const char * const RPDCHAIN_PID_FILENAME = "rpdchain.pid";
const char * const RPDCHAIN_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\RPDCHAIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\RPDCHAIN
// Mac: ~/Library/Application Support/RPDCHAIN
// Unix: ~/.rpdchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RPDCHAIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RPDCHAINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RPDCHAINParams
    // Mac: ~/Library/Application Support/RPDCHAINParams
    // Unix: ~/.rpdchain-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RPDCHAINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "RPDCHAINParams";
#else
    // Unix
    return pathRet / ".rpdchain-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", RPDCHAIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", RPDCHAIN_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty rpdchain.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override rpdchain.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", RPDCHAIN_PID_FILENAME));
