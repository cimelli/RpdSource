...
v0.0.10 is a major release and consensus critical in terms of the Token Layer protocol rules. An upgrade is mandatory, and highly recommended. Prior releases will not be compatible with new behavior in this release.
...
- [Token Core v0.0.10](#token-core-v0010)
...
- [Imported changes and notes](#imported-changes-and-notes)
  ...
  - [Rebranding to Token Core](#rebranding-to-token-core)
...
- [Other notable changes](#other-notable-changes)
  ...
  - [CI and testing of Token Core](#ci-and-testing-of-token-core)
...
Downgrading to an Token Core version prior 0.0.10 is generally not supported as older versions will not provide accurate information due to the changes in consensus rules.
...
The underlying base of Token Core was upgraded from Bitcoin Core 0.9.5 to Bitcoin Core 0.10.4.

Please see the following release notes for further details:

- [Release notes for Bitcoin Core 0.10.0](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/doc/release-notes/release-notes-0.10.0.md)
- [Release notes for Bitcoin Core 0.10.1](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/doc/release-notes/release-notes-0.10.1.md)
- [Release notes for Bitcoin Core 0.10.2](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/doc/release-notes/release-notes-0.10.2.md)
- [Release notes for Bitcoin Core 0.10.3](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/doc/release-notes/release-notes-0.10.3.md)
- [Release notes for Bitcoin Core 0.10.4](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/doc/release-notes.md)
...
The minimum relay fee influences the "dust threshold", and has an impact on the output values of Token transactions, which are chosen to be as low as possible. As per default, Token transactions created with Master Core had output values between `0.00000546` BTC and `0.00000882` BTC, whereby the new output values are between `0.0000273` BTC and `0.0000441` BTC.

To continue to create transactions with lower values, start Token Core with `-minrelaytxfee=0.00001` or add the following to your `bitcoin.conf`:
```
...
Rebranding to Token Core
-----------------------

Master Core was re-branded to Token Core on all levels:

- `mastercored`, `mastercore-cli`, `mastercore-qt`, `test_mastercore` and `test_mastercore-qt` were renamed to `tokencored`, `tokencore-cli`, `tokencore-qt`, `test_tokencore` and `test_tokencore-qt`
- the debug log file `mastercore.log` was renamed  to `tokencore.log`
- the hardcoded token SP#1 was renamed from `"MasterCoin"` to `"Token"`
- the hardcoded token SP#2 was renamed from `"Test MasterCoin"` to `"Test Token"`
- the user interface now refers to `"Token"`, `"TOKEN"`, `"Test Token"` or `"TTOKEN"` instead of `"Mastercoin"`, `"MSC"`, `"Test Mastercoin"` or `"TMSC"`
...
Token Core 0.0.10 introduces feature activations, a new concept that allows the Token team to decouple the release process from the process of making new features live.

Prior to 0.0.10, the block height that a new feature would be made live was hard coded, placing substantial limits on both the frequency of releases and the flexibility in introducing new features. The feature activations system allows to separate the release process and activation of features by allowing the Token team to activate features with a special Token transaction instead of with a hard coded block height within a specific release.
...
- http://blog.token.foundation

As per default Token Core accepts activation messages from the following source:
```
{
...
The activation message must be signed by at least 4 of the 5 nominated members of the Token team in order to be accepted. These members are as follows:
...
| Min. client version        | 32-bit unsigned | 1000000 (Token Core 0.0.10)         |
...
Senders of activation messages can be whitelisted or blacklisted with the configuration options `-tokenalertallowsender=<sender>` and `-tokenalertignoresender=<sender>`. Please be aware that overriding activations may cause the client to provide inaccurate data that does not reflect the rest of the network.

The GUI displays warnings for pending activations and daemon users can view pending and completed activations via the [token_getactivations](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_getactivations) JSON-RPC API call.
...
Please note, while Token Core 0.0.10 contains support for several new rules and features they are not enabled immediately and will be activated via the feature activation mechanism described above.
...
Token Core 0.0.10 contains support for Class C transactions which move the data encoding from bare-multisig data outputs to an `OP_RETURN` output. `OP_RETURN` outputs carry no output value and can be immediately pruned.

In addition to migrating data encoding to `OP_RETURN`, the requirement to send an output to the Exodus address has also been removed with Class C. Instead the bytes `0x6f6d6e69` ("token") are prefixed to the data payload to provide marker identification.
...
The distributed token exchange, or "MetaDEx", was integrated into Token Core, to support trading of tokens with automated order matching.
...
| Tokens desired in exchange | 32-bit unsigned | 2 (Test Token)                      |
...
See also: [token_sendtrade](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_sendtrade), [token_sendcanceltradesbyprice](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_sendcanceltradesbyprice), [token_sendcanceltradesbypair](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_sendcanceltradesbypair), [token_sendcancelalltrades](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_sendcancelalltrades), [gettokentrade](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokentrade), [gettokentrade](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokentrade), [gettokentradehistoryforpair](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokentradehistoryforpair), [gettokentradehistoryforaddress](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokentradehistoryforaddress) JSON-RPC API calls
...
See also: [token_sendall](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_sendall) JSON-RPC API call
...
The JSON-RPC API was extended significantly, and now supports sending transactions of any transaction type, and the creation of raw Token transactions.

Several new API calls are available to retrieve information about unconfirmed or confirmed Token transactions, and the state of the system.

Please see the JSON-RPC API documentation for more details:

- https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md

Debug logging categories
------------------------

Debug logging is categorized and default logging options may be changed with the `-tokendebug` parameter. Use multiple `-tokendebug` parameters to specify multiple categories.
...
- `packets`: Log additional information about the payload included within each Token transaction
- `packets_readonly`: Log additional information about the payload included within each Token transaction when parsing in read only mode (e.g. via RPC interface)
...
Usage examples:
```
tokencored -tokendebug=all
```
```
tokencored -tokendebug=parser -tokendebug=parser_data -tokendebug=vin -tokendebug=script
```

Configuration options
---------------------

For a listing of other Token Core specific configuration options see:

- https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/configuration.md

Various performance improvements
--------------------------------

Token Core was improved on several levels to increase performance and responsiveness.
...
The serialization of token related database entries was previously JSON based, and the time to retrieve information about transactions, tokens or crowdsales, for example with [token_gettransaction](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_gettransaction), [token_getproperty](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#token_getproperty), [gettokencrowdsale](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokencrowdsale) or [gettokentrade](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#gettokentrade), was primarily delayed by slow conversions of the data stored in the database. Entries are now stored in their raw byte representation, which allows much faster deserialization of the data. Benchmarks have shown this update provides a speed improvement of about 8x or more for these calls.
...
It is mandatory that the state of the system is similar for all participants, and Token Core 0.0.10 introduces the concept of "consensus hashes", which are a compact representation of the state. This allows to compare the state of the system with other participants, and to compare historical state with known reference states, to ensure consensus.

To log consensus hashes, the configuration options `-tokendebug=consensus_hash` and `-tokendebug=consensus_hash_every_block` can be used.

Checkpoints and faster initial transaction scanning
---------------------------------------------------

When Token Core is used the very first time, or when Token Core hasn't been used for a longer period, then historical blocks are scanned for Token transactions. With the introduction of "consensus hashes", a significant speed improvement during the initial stage was possible by skipping known blocks without Token transactions, and by comparing the resulting state with checkpoints. If the state of the client doesn't match the reference state, the client shuts down to prevent providing inaccurate data.

Token Core was intentionally not delivered with a snapshot of the most recent state, and transactions are fully verified in any case. One design goal of Token Core has always been to minimize the role of central parties (in this case: the ones providing a snapshot or checkpoints), and to further minimize the dependency on a hardcoded list of information, the configuration option `-tokenseedblockfilter=0` can be used to disable the skipping of blocks without Token transactions.

The configuration option `-overrideforcedshutdown` can be used to prevent Token Core from shutting down in case of a checkpoint mismatch. Using the latter is generally not recommended, as this exposes the user to data, which isn't considered as valid by other participants.

Disable-wallet mode
-------------------

Since this version Token Core can be build and used without wallet support.

This can be handy, when using Token Core as plain data provider, without the need to send transactions. [Data retrieval](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#data-retrieval) and [raw transaction](https://github.com/TokenLayer/tokencore/blob/tokencore-0.0.10/src/tokencore/doc/rpc-api.md#raw-transactions) API calls are enabled in this mode.

To compile Token Core without wallet support (illustrated for Unix):
```
./autogen.sh
...
To disable to wallet via configuration options start Token Core with `-disablewallet`, or add the following to your `bitcoin.conf`:
```
disablewallet=1
```

CI and testing of Token Core
---------------------------

As of now, Token Core 0.10 has nearly 5000 lines of source code for unit tests in addition to the unit tests inherited from Bitcoin Core. Furthermore [TokenJ](https://github.com/TokenLayer/TokenJ#token-integration-testing-with-spock-framework) is used for integration testing.

To run the unit tests locally, start `test_tokencore` or `test_tokencore-qt`. On Unix, the integration tests can be started with `qa/pull-tester/tokencore-rpc-tests.sh`. Please note, the script for the integration tests is not included in the release packages. For other operating systems, or to manually run the integration tests, see the [instructions for TokenJ](https://github.com/TokenLayer/TokenJ#token-integration-testing-with-spock-framework).

Travis CI was integrated into the work flow, to automatically build and test Token Core on several different platforms and operating systems.
...
- #13 Support creation of any (enabled) Token transaction via RPC
...
- #42 Integrate Travis CI and TokenJ into workflow
...
- #109 Log every invalid processing of Token transactions
- #91 Rebrand project to Token Core
- #91 Rename files to `tokencore-qt`, `tokencored`, `tokencore-cli`, ...
...
- #128 Run TokenJ tests with all build targets except OS X
...
- #149 Update Token Core API documentation
...
- #178 Fix Token transaction count value passed into block end handler
- #179 Add consensus checkpoint for block 370000
- #180 Update seed blocks to 370000
- #181 Update base to Bitcoin Core 0.10~ tip
- #174 Add RPC support for unconfrimed Token transactions
...
- #217 Rebrand "Mastercoin" to "Token"
...
- #232 Add new RPC to list pending Token transactions
...
- #250 Fix missing crowdsale purchase entries in verbose "gettokencrowdsale"
- #253 Update handling of crowdsales and missing bonus amounts
- #142 Add RPC to decode raw Token transactions
...
- #262 Filter empty balances in token_getall* RPC calls
...
- #278 Replace splashes to remove TokenWallet branding
- #269 Add release notes for Token Core 0.0.10
...
- #286 Bump version to Token Core 0.0.10-rc3
- #288 Expose payload over RPC and add payload size
- #291 Add error handlers for "token_getpayload"
- #292 Add API documentation for "token_getpayload"
- #295 Fix overflow when trading divisible against divisible
- #303 Force UI update every block with Token transactions
- #305 Change default confirm target to 6 blocks
- #307 Update release notes for 0.0.10
- #306 Bump version to Token Core 0.0.10-rel
```
