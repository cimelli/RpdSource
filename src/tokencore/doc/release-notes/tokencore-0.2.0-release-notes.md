...
This version is built on top of v0.0.12, which is a major release and consensus critical in terms of the Token Layer protocol rules. If you are using an older version of Token Core than v0.0.12, an upgrade is mandatory, and highly recommended. Prior releases will not be compatible with new behavior in this release.
...
- [Token Core v0.2.0](#token-core-v020)
...
Downgrading to an Token Core version prior 0.0.12 is generally not supported as older versions will not provide accurate information due to the changes in consensus rules. Downgrading to Token Core 0.0.12 can require a reindex of the blockchain, and is not recommended.
...
The underlying base of Token Core was upgraded from Bitcoin Core 0.10.4 to Bitcoin Core 0.13.2.
...
Earlier versions of Token Core (prior to 0.2.0) used Bitcoin Core 0.10.x as a base. Token Core 0.2.0 however is based on a much newer version of Bitcoin Core (0.13.2) and thus inherits various changes and improvements to the handling of fees that have been added to Bitcoin Core over time.

It is highly recommended that users of Token Core consider these fee changes and their chosen fee settings when upgrading to Token Core 0.2.0, and test thoroughly to ensure that fee behavior is desirable and as expected.

Consideration of the modified behavior for the `-paytxfee` setting is especially important. Earlier versions of Bitcoin Core (and thus earlier versions of Token Core prior to 0.2.0) would round the size of the transaction upwards to the nearest kilobyte when calculating the fee (for example a 250 byte transaction would be rounded up to 1 kB). This issue has been resolved in newer versions of Bitcoin Core, and so Token Core 0.2.0 will no longer perform this rounding when calculating the fee. A comparison of the behaviors can be shown in the following, where an example `paytxfee` value of 0.001 BTC/kB has been set:

- Token Core prior to 0.2.0: A transaction with a size of 250 bytes will be rounded up to 1 kB, and so a fee of 0.001 BTC will be used
- Token Core 0.2.0: A transaction with a size of 250 bytes will not be rounded, and so a fee of 0.00025 BTC will be used

It is also worth noting that the fee estimation algorithms were updated, and thus the fees chosen when using `-txconfirmtarget` (along with the output of the `estimatefee` RPC) will likely be different in Token Core 0.2.0 when compared to prior versions.
...
Starting with this version of Token Core, the versioning scheme becomes more intuitive, as it uses MAJOR.MINOR.PATCH from now on, whereby MAJOR indicates consensus affecting changes or other non-backwards-compatible changes, MINOR indicates new functionality in a backwards-compatible manner, and PATCH is used to indicate backwards-compatible bug fixes.

New project branch structure
----------------------------

The latest stable version of Token Core can be found in the [master](https://github.com/TokenLayer/tokencore/tree/master) branch on GitHub, while the version under development can be found in the [develop](https://github.com/TokenLayer/tokencore/tree/develop) branch. This provides a cleaner seperation of source code.
...
Please note, while Token Core 0.2.0 contains support for several new rules and features they are not enabled immediately and will be activated via the feature activation mechanism described above.
...
In earlier version of Token Core (prior to 0.2.0), when creating transactions with the Qt UI or the JSON-RPC API (for example with `token_send`), then the coin selection algorithm may have selected unspent outputs, which are not economic to spend. This may have caused the creation of larger and more expensive transactions than necessary.

In Token Core 0.2.0 this is addressed by excluding inputs during the transaction creation, which are more expensive to spend than they are worth. Please note the exclusion is directly related to the fee related configuration options of Token Core, such as `-paytxfee` or `-txconfirmtarget`.
...
To further speed up the inital parsing process, blocks without Token transactions are skipped up until block 460,000. To avoid relying on a hardcoded list of seed blocks, Token Core can be started with `-tokenseedblockfilter=0`.

Easy access to specific consensus hashes when parsing
-----------------------------------------------------

Previously to confirm a consensus hash for a particular block it was required to enable `-tokendebug=consensus_hash_every_block` during parsing to log the hash for every block which caused a significant slow down due to the extra work involved.
...
This version adds a `-tokenshowblockconsensushash` startup option which can be used to generate consensus hashes for specific blocks.

For example, to validate the checkpoint for block 450,000 without using seed block filtering we can use:

```
./tokencored --startclean --tokenseedblockfilter=false --tokenshowblockconsensushash=450000
```
...
- #454 Fix two Token Core related build warnings
...
- #460 Bump version to Token Core 0.2.0 and add release notes
```
