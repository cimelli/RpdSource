...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in rpdchain/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in rpdchain/kb\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"rpdchainaddress\"\n"
            "\nReturn information about the given rpdchain address.\n"

            "\nArguments:\n"
            "1. \"rpdchainaddress\"     (string, required) The rpdchain address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"type\" : \"xxxx\",              (string) \"standard\" or \"sapling\"\n"
            "  \"address\" : \"rpdchainaddress\",    (string) The rpdchain address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for RPDCHAIN cold staking -only if is standard address-\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("validateaddress", "\"RPSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\"") +
            HelpExampleCli("validateaddress", "\"sapling_address\"") +
            HelpExampleRpc("validateaddress", "\"RPSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\""));
...
#ifdef ENABLE_WALLET
        // Case 1: RPDCHAIN address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are rpdchain addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) rpdchain address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
            ...
            HelpExampleCli("createmultisig", "2 \"[\\\"R6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\",\\\"R71sgjn4YtPu27adkKGrdDwzRTxnRkBfKV\\\"]\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("createmultisig", "2, \"[\\\"R6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\",\\\"R71sgjn4YtPu27adkKGrdDwzRTxnRkBfKV\\\"]\""));
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"rpdchainaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"rpdchainaddress\"  (string, required) The rpdchain address to use for the signature.\n"
            ...
            HelpExampleCli("signmessage", "\"RD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"my message\"") +
            "\nVerify the signature\n" +
            HelpExampleCli("verifymessage", "\"RD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"signature\" \"my message\"") +
            "\nAs json rpc\n" +
            HelpExampleRpc("verifymessage", "\"RD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\", \"signature\", \"my message\""));
