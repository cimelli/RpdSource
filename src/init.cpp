...
void PrepareShutdown()
{
    ...
    util::ThreadRename("rpdchain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), RPDCHAIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), RPDCHAIN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), RPDCHAIN_MASTERNODE_CONF_FILENAME));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The RPDCHAIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("rpdchain-loadblk");
...
/** Sanity checks
 *  Ensure that RPDCHAIN is running in a usable environment with all
...
#endif
    LogPrintf("RPDCHAIN version %s (%s)\n", version_string, CLIENT_DATE);
}
...
bool AppInit2()
{
    ...
    //if (GetBoolArg("-masternode", DEFAULT_MASTERNODE) && !GetBoolArg("-listen", DEFAULT_LISTEN))
        ...
        //LogPrintf("Silly bird you are most likely trying to run more than one masternode per vps right?");
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. RPDCHAIN Core is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single RPDCHAIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. RPDCHAIN Core is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //RPDCHAIN specific: zerocoin and spork DB's
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zrpd if already reindexed
                    RecalculateRPDSupply(1, fReindexZerocoin);
                }
