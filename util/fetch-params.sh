...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/RPDCHAINParams"
    else
        PARAMS_DIR="$HOME/.rpdchain-params"
    fi
fi
...
Failed to fetch the RPDCHAIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
RPDCHAIN - fetch-params.sh

This script will fetch the RPDCHAIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common RPDCHAIN zkSNARK parameters. Note that it is
